...
function ActivityPageInner() {
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      {/*
        Every tab stays mounted (just hidden) instead of being torn down on
        switch, so reopening a tab shows already-current data instantly
        instead of a fresh loading spinner. Each tab's own polling still
        pauses while it isn't the visible one (via the `active` prop) — every
        row's background job poll plus each tab's own list poll running
        continuously regardless of which tab was showing used to produce
        constant network activity even while sitting on an unrelated tab.
      */}
