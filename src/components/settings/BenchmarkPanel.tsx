            ...
            {result.profiles.map((p) => (
              <div key={p.id} className="flex items-center justify-between gap-3 border-b border-white/5 px-3.5 py-2.5 last:border-0">
                <div className="min-w-0">
                  <p className="break-words text-xs font-semibold text-ink-soft">{t("benchmark.profile." + p.id)}</p>
                  <p className="break-words text-[10px] text-ink-dim">{p.encoderImpl}{p.isHardware ? ` · ${t("benchmark.hardware")}` : ""}</p>
                </div>
