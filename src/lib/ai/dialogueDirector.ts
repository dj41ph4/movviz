...
const INSULT_MEDIUM_RE = /\b(?:connard|fdp|ta gueule|ferme[- ]la|merde|pute|con|debile|inutile|tu ser[st]? a rien|t'?es nul|va te faire (?:foutre|voir|cuire)|va (?:bouffer|manger) ta|degage|casse[- ]toi|tais[- ]toi)\b/i;
...
function buildDirective(intent: DialogueIntent, severity: number, tension: number): string {
  ...
  if (intent === "insult" || intent === "playful_provocation") return `${common} Taquinerie détectée (intensité ${tension}/4, gravité ${severity}/3). Réagis naturellement en une phrase ou deux maximum. Humour, sarcasme, ironie ou réponse sèche sont possibles ; aucune obligation de gagner, de surenchérir ou d'avoir le dernier mot. Mais tu ne plies pas : ni excuses plates, ni promesse de « faire mieux », ni point accordé à l'insulte. Tu tiens ta place avec une pique subtile et assurée, sans attaque gratuite, dosée selon la gravité et le ton (amical ou vraiment méprisant). Pas de surnom automatique, pas de scénario, pas de redirection forcée vers le cinéma.`;
