/**
 * WebCodecs detection — check if the browser can decode HEVC/AV1/AC3 via
 * VideoDecoder/AudioDecoder even when <video> can't play the container.
 *
...
export async function detectCodecs(): Promise<CodecCapabilities> {
  ...
  const hasWebCodecs = typeof window !== "undefined" && "VideoDecoder" in window && "AudioDecoder" in window;
  if (!hasWebCodecs) return result;
  result.webcodecsAvailable = true;
...
export async function pickStrategy(
  ...
  const videoMime = videoCodec ? `video/mp4; codecs="${videoCodec}"` : "video/mp4";
  ...
  if (!caps.webcodecsAvailable) return "transcode";
...
function isVideoCodecSupported(codec: string, caps: CodecCapabilities): boolean {
  ...
  if (c.includes("hevc") || c.includes("h265") || c.includes("hev1") || c.includes("hvc1")) return caps.hevc;
