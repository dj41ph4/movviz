  ...
  const written = withUserContextDb((db) => {
    db.prepare(`
      ...
      ON CONFLICT(state_key) DO UPDATE SET
        ...
        watched_source = CASE WHEN excluded.watched_updated_at IS NOT NULL AND (user_media_state.watched_updated_at IS NULL OR excluded.watched_updated_at > user_media_state.watched_updated_at) THEN excluded.watched_source ELSE user_media_state.watched_source END,
