...
class MovvizRepository(private val baseUrl: String) {
    ...
    suspend fun tvPreview(type: String, tmdbId: Int): ApiResult<TvPreviewDto> =
        safeCall { api.tvPreview(type, tmdbId) }
    ...
    suspend fun search(query: String, type: String? = null): ApiResult<List<SearchResultDto>> {
        return when (val response = searchPage(query, page = 1, type = type)) {
            is ApiResult.Success -> {
    ...
    private suspend fun searchPage(query: String, page: Int, type: String?): ApiResult<SearchResponseDto> {
        val startedAt = SystemClock.elapsedRealtime()
        return try {
            val response = api.search(query, page, type)
