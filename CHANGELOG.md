...
- Movviz range désormais l'anthologie comme Plex l'attend, dans le dossier « Monster (2022) » : Dahmer en saison 1, les frères Menendez en saison 2, Ed Gein en saison 3 et Lizzie Borden en saison 4. Les fichiers déjà téléchargés (Lizzie Borden, Ed Gein) sont déplacés automatiquement une minute après la mise à jour, puis Plex est relancé. Quand le NAS refuse un simple déplacement entre deux dossiers, le fichier est copié, vérifié, puis l’original supprimé. Dans Movviz, chaque volet reste une série à part, avec sa propre fiche.
...
- Aucun changement visuel par rapport à v1.25.18.
- Les seules images dont le cache est forcé sont les photos de profil.
- Un identifiant de session est ajouté à l'URL de l'avatar à chaque nouveau démarrage du process TV : Coil recharge donc réellement la photo au lieu de réutiliser une ancienne image en cache.
- L'URL reste stable pendant toute la session afin d'éviter des téléchargements répétés lors des recompositions.
- Le cache-buster éphémère n'est jamais persisté dans les préférences locales.
