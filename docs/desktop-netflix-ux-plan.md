...
| 2026-08-22 | 3 | Correctif de sémantique du cache : l'incrémental quotidien ne se limite plus aux absences de métadonnées. Il parcourt durablement la bibliothèque par tranches de 30, titres existants inclus, pour vérifier/rétablir les octets locaux sans retélécharger les fichiers immuables déjà sains. Chaque carte réellement affichée continue d'inscrire ses propres fichiers dans le même cache. |
| 2026-08-22 | 3–4 | Preview de survol alignée sur la référence : l'image elle-même ouvre la fiche unique, sans bouton Informations. Actions compactes Play ou Ajouter selon la présence réelle, J'aime = note explicite 5/5, flèche vers la fiche ; année/durée en badges et trois genres au besoin enrichis paresseusement au survol. Les métadonnées restent `fr-FR` ; TMDb ne fournit toutefois pas de code régional distinct pour les images, donc les logos français restent techniquement étiquetés `fr`. |
| 2026-08-22 | 3–4 | Stabilité : le verrou de défilement du panel et celui du lecteur sont désormais partagé et compté. Les sorties Framer Motion qui se chevauchent ne peuvent plus laisser le `body` en `overflow: hidden`. Le choix du fond neutre favorise aussi le 16:9, la résolution et la validation communautaire TMDb. La migration de cache conserve les couples backdrop/logo v1 pour qu’un reboot ou une mise à jour ne mette jamais les cartes à nu. |
| 2026-08-22 | 3 | Correctif de persistance logique : le registre de caches nommés est ancré sur `globalThis`, conformément aux règles Next/Movviz. Les différents bundles serveur ne peuvent plus créer chacun un faux cache TMDb avec ses propres compteurs et son propre état après un reboot. |
