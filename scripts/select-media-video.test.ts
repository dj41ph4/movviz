...
test("carousel : un Trailer officiel plus récent ne bat jamais un Teaser valide (règle absolue)", () => {
  const videos = [
  ...
  assert.equal(result[0], "teaser-plain-old");
});

test("details : le Trailer prend la priorité sur le Teaser (miroir exact du carrousel)", () => {
  const videos = [
