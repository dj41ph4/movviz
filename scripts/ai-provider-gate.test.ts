...
test("calls to the same provider run one at a time, spaced for Mistral's free tier", async () => {
  ...
  try {
    const cfg = config("mistral", "mistral-small-latest");
    await Promise.all([
      callAi(cfg, "system", [{ role: "user", content: "a" }]),
      callAi(cfg, "system", [{ role: "user", content: "b" }]),
    ]);
    assert.equal(maxInFlight, 1);
    assert.ok(starts[1] - starts[0] >= 1_050, `spacing was ${starts[1] - starts[0]} ms`);
  } finally {
...
test("a provider still refusing after its retry is left alone instead of hammered", async () => {
  ...
  try {
    const cfg = config("openrouter", "openrouter/free");
